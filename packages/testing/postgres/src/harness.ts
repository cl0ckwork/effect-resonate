...
export interface PostgresTestContext {
  ...
  readonly integresqlPort: number
  readonly templateHash: string
}

export const createHarness = ({ postgresPort, integresqlPort }: Omit<PostgresTestContext, "templateHash">) => {
  ...
  const initializeTemplate = async (): Promise<string> => {
  ...
  const withDatabase = async (templateHash: string, run: (url: string) => Promise<void>): Promise<void> => {
  ...
  const scenarioLayer = (url: string) => Layer.merge(
  ...
  return { controlUrl, initializeTemplate, withDatabase, scenarioLayer }
}

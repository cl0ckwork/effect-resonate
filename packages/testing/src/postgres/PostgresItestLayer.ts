...
const withControlClient = <A>(options: {
  ...
  const client = yield* Effect.acquireRelease(
    Effect.sync(() => new Client({ connectionString: controlDatabaseUrl({ postgresPort: options.postgresPort }) })),
...
const scheduleTimeouts = (options: {
  ...
  use: (client) => Effect.promise(async () => {
    await client.query(
...
const unscheduleTimeouts = (options: { readonly postgresPort: number; readonly jobName: string }) =>
  withControlClient({
    postgresPort: options.postgresPort,
    use: (client) => Effect.promise(async () => {
      const jobs = await client.query<{ jobid: string }>("SELECT jobid FROM cron.job WHERE jobname = $1", [options.jobName])
...
const testDatabase = Effect.gen(function*() {
  ...
  const lease = yield* Effect.acquireRelease(
    Effect.promise(() => integresql.getTestDatabase(env.templateHash)).pipe(
      Effect.map((database) => ({
        connectionString: dbConfigToHostUrl({ client: integresql, config: database, postgresPort: env.postgresPort }),
  ...
  yield* scheduleTimeouts({ postgresPort: env.postgresPort, database: lease.database, jobName: lease.jobName })
...
export const makePostgresItestLayer = (options: PostgresItestOptions = {}) =>
  Layer.unwrap(Effect.gen(function*() {
    ...
    return Layer.merge(
      PostgresNetwork.layer({ connectionString, ...options.network }),

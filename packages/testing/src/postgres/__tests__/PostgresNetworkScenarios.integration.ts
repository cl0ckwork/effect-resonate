...
describe("PostgresNetwork with the U5 conformance scenarios", () => {
  itest(
    "completes a registered workflow containing an Effect step",
    () => NetworkScenarios.completion
  )
  itest(
    "preserves first-writer-wins under duplicate activation",
    () => NetworkScenarios.duplicateActivation
  )
  itest(
    "keeps checked failures, defects, and malformed input distinct",
    () => NetworkScenarios.failureClassification
  )
  itest(
    "keeps durable execution alive after local cancellation",
    () => NetworkScenarios.cancellation
  )
  itest("prevents late resolution from replacing a durable timeout", () => NetworkScenarios.timeout)
  itest("supports repeated worker stop and restart", () => NetworkScenarios.lifecycle)
  itest(
...
describe("real Postgres acquisition failures", () => {
  it.live("reports a missing Resonate schema", () =>
    Effect.gen(function* () {
  ...
  it.live("reports a failed database login", () =>
    Effect.gen(function* () {
      const { postgresPort } = yield* PostgresTestEnv
      yield* expectFailure({
        connectionString: controlDatabaseUrl({ postgresPort }).replace(
          "effect_resonate_test_password",
          "wrong-password"
        )
      })
    }).pipe(Effect.provide(PostgresTestEnv.layer))

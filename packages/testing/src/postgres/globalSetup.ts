...
const compose = (options: {
  ...
  Effect.tryPromise((signal) =>
    execFileAsync(
      "docker",
      ["compose", "-f", composeFile, "-p", options.projectName, ...options.command],
...
const waitForIntegresql = ({ url }: { readonly url: string }) =>
  Effect.tryPromise((signal) =>
    fetch(new URL("api/v1/templates/__readiness__/tests", url), { signal }).then((response) => {
      if (response.status >= 500)
        throw new Error(`IntegreSQL readiness returned ${response.status}`)
    })
  ).pipe(
  ...
  Effect.scoped(
    Effect.gen(function* () {
      ...
      const schema = yield* Effect.promise(() =>
        readFile(fixture({ name: "resonate.sql" }), "utf8")
      )
      yield* Effect.promise(() => client.query(schema))
      const compatibility = yield* Effect.promise(() =>
        readFile(fixture({ name: "001-sdk-global-promise.sql" }), "utf8")
      )
      yield* Effect.promise(() => client.query(compatibility))
      const check = yield* Effect.promise(() =>
        client.query<{ version: string | null }>("SELECT resonate.get_schema_version() AS version")
      )
      if (check.rows[0]?.version !== options.expectedSchemaVersion) {
        return yield* Effect.fail(
          new Error("Resonate SQL schema version does not match POSTGRES_TEST_SCHEMA_VERSION")
        )
      }
    })
  )

const setupEffect = ({ project }: { readonly project: TestProject }) =>
  Effect.gen(function* () {
    ...
    yield* Effect.gen(function* () {
      ...
      const postgresPort = yield* publishedPort({
        projectName,
        service: "postgres",
        containerPort: 5432
      })
      const integresqlPort = yield* publishedPort({
        projectName,
        service: "integresql",
        containerPort: 5000
      })
      ...
      yield* Effect.promise(() =>
        integresql.initializeTemplate(templateHash, (database) =>
          Effect.runPromise(
            migrate({

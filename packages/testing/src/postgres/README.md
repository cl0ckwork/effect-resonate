...
Compose publishes both services on Docker assigned loopback host ports. Global setup discovers the assigned ports and waits for the IntegreSQL API before preparing the template, so an existing local Postgres or IntegreSQL process does not claim the same host port. Compose commands have a ten-minute process timeout. The schema version check expects `0.1.0` for the pinned SQL fixture; set `POSTGRES_TEST_SCHEMA_VERSION` when testing a fixture with another version.

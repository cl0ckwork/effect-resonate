...
The test template applies this file as a database migration before IntegreSQL finalizes it. It then applies `001-sdk-global-promise.sql`, a compatibility migration for published SDK 0.11.5: `ctx.promise()` carries `resonate:scope=global`, but this SQL revision does not classify that tag as externally awaitable. Without the patch, `task.suspend` can return 422 during recovery. The [SDK source fix](https://github.com/resonatehq/resonate-sdk-ts/commit/a3a3ccf3f92c636afcf51d4f10c4ad44d2b5399b) adds `resonate:external=true`; it has not reached the published SDK used by these tests (0.11.5). Remove the migration once a published SDK includes that fix and the recovery test passes without it. IntegreSQL's `hashFiles` covers the migration fixtures. Update the upstream revision, fixture, migration, and compatibility tests together.

The image derives from the pinned official Postgres 16 Bookworm image and installs `postgresql-16-cron`; the IntegreSQL image is pinned by digest in `packages/testing/docker-compose.yml`.

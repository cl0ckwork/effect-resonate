...
const makeUnitHarness = (options: {
  ...
  acquire: Effect.succeed({
    ...
    capabilities: {},
    observe: ({ executionIds }: NetworkHarness.ObservationRequest) => options.observationDefect === undefined
      ? Effect.succeed({
  ...
  it("releases earlier setup when a later acquisition step fails and sanitizes the error", async () => {
    ...
    const Failing = NetworkHarness.layer({
      ...
      acquire: Effect.acquireRelease(
        ...
        Effect.andThen(Effect.fail(new Error(secret)))
      )
    })

    const failure = await Effect.runPromise(Effect.flip(
      NetworkHarness.NetworkHarness.pipe(Effect.provide(Failing))
    ))

...
const fixtureLayer = (options: {
...
}): Layer.Layer<LocalFixture> => Layer.effect(
  LocalFixture,
  Effect.acquireRelease(
    ...
    (fixture) => Effect.promise(() => fixture.network.stop()).pipe(
...
const runWithLocalNetwork = (
  scenario: Effect.Effect<
    ...
    NetworkHarness.NetworkHarness | ResonateNetwork.ResonateNetwork
  >
): Effect.Effect<void, NetworkHarness.ConformanceFailure> => Effect.gen(function*() {
...
const expectMissingCapability = (options: {
  ...
  const failure = yield* Effect.flip(options.scenario.pipe(
    Effect.provide(makeLocalNetworkTestLayer({ capabilities: {} }))
  ))
...
describe("SDK LocalNetwork conformance self-test", () => {
  ...
  it.live("rejects durable operations after an ordinary await", () =>
    runWithLocalNetwork(NetworkScenarios.invalidNonDurableAwait), 15_000)

  it.live("reuses a checkpointed child after worker replacement", () =>
    runWithLocalNetwork(replayRecovery), 15_000)

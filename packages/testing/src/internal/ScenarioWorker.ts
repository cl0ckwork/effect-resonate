...
const workerFailure = (
  ...
  issue: "WorkerFailed" | "CleanupFailed"
) => fail({
  scenario: options.scenario,
  issue,
  assertion: issue === "WorkerFailed"
...
export const startWorker = (
  options: StartWorkerOptions
): Effect.Effect<ScenarioWorker, ConformanceFailure, NetworkHarness | Scope.Scope> => Effect.gen(function*() {
  const harness = yield* NetworkHarness
  const runtime = yield* Effect.acquireRelease(
    Effect.sync(() => ManagedRuntime.make(options.layer)),
    (runtime) => runtime.disposeEffect.pipe(
  ...
  const ready = yield* Effect.promise((signal) => runtime.runPromiseExit(ResonateClient, { signal })).pipe(
    Effect.timeoutOption(harness.timing.scenarioTimeout),
    Effect.flatMap(Option.match({
  ...
  return {
    runExit: ({ effect }) => Effect.promise((signal) => runtime.runPromiseExit(effect, { signal })).pipe(
      Effect.timeoutOption(harness.timing.scenarioTimeout),
      Effect.flatMap(Option.match({
        onNone: () => workerFailure(options, "WorkerFailed"),
        onSome: Effect.succeed

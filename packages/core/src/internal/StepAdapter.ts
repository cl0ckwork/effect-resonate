...
const metadata = (context: Info): StepContextService =>
  Object.freeze({
    id: context.id,
...
const rejectedReason = <E>(
  cause: Cause.Cause<E>
): "Defect" | "Interrupted" | "CompositeCause" | "Unknown" => {
...
const rejected = <Definition extends Step.Any>(
  ...
  _tag: "Rejected",
  throwable: cause instanceof Error ? cause : DurableRejection.make(definition, executionId, reason)
})

const classify = <Definition extends Step.Any>(
  definition: Definition,
  executionId: string,
  ...
  Match.valueTags(exit, {
    ...
    Failure: ({ cause }) => {
      ...
      const failure =
        cause.reasons.length === 1 && Cause.isDieReason(cause.reasons[0]!)
  ...
  async (
    ...
    const program = Effect.gen(function* () {
      ...
      const exit = yield* handler
        .execute(input.success as Step.Step.Input<Definition>, metadata(context))
        .pipe(Effect.exit)

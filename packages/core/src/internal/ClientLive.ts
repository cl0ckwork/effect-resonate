...
): Effect.Effect<Value, ResonateSdkError> =>
  Effect.tryPromise({
    try: evaluate,
  ...
  > =>
    sdkEffect("handle.result", true, () => handle.result()).pipe(
      ...
      Effect.flatMap((value) =>
        Effect.fromResult(DurableOutcome.decodeWorkflowResult(workflow, handle.id, value))
      ),
...
): Effect.Effect<ResonateHandle<unknown>, ResonateSdkError> =>
  sdkEffect(operation, true, () => resonate[operation]<unknown>(id, func, ...args)).pipe(
    Effect.map(rawHandle)
  )
...
const get = (
  ...
  sdkEffect("get", false, () => resonate.get(id)).pipe(
    Effect.map((handle) =>
      workflow === undefined ? rawHandle(handle) : typedHandle(workflow, handle)
    )
  )
...
const registeredFunction = <Func extends AnyFunc>(
  ...
  run: (...args) =>
    sdkEffect("run", true, () => registered.run(...args)).pipe(Effect.map(rawHandle)) as ReturnType<
      ResonateFunc<Func>["run"]
    >,
  rpc: (...args) =>
    sdkEffect("rpc", true, () => registered.rpc(...args)).pipe(Effect.map(rawHandle)) as ReturnType<
      ResonateFunc<Func>["rpc"]
    >,
    ...
    for (const definition of definitions) {
      Match.value(definition).pipe(
        Match.discriminators("kind")({
          Step: (step) => {
            resonate.register(step.name, StepAdapter.make(step, runPromise), {
...
): Effect.Effect<void, ResonateSdkError, AdapterSupervisor> =>
  Effect.gen(function* () {
    gate.close()
    ...
    yield* Option.match(drained, {
      onNone: () =>
        AdapterSupervisor.abandon.pipe(
...
export const make = <Group extends ResonateFunctions.Any>(
  ...
  ResonateClientService,
  DefinitionRegistry.Error | InvalidClientConfiguration | ResonateSdkError,
...
> =>
  Effect.gen(function* () {
    ...
    const parsedDrainTimeout = Duration.fromInput(options.drainTimeout).pipe(
      ...
      Option.filter(
        () => typeof options.drainTimeout !== "number" || Number.isFinite(options.drainTimeout)
      )
    )
    ...
    const promiseSettle = (
      ...
      Schema.isSchema(args[1])
        ...
        : sdkEffect(`promises.${operation}`, true, () =>
            (
              resonate.promises[operation] as (
    ...
    const promises = {
      ...
      resolve: ((...args: ReadonlyArray<unknown>) =>
        promiseSettle("resolve", args)) as PromisesService["resolve"],
      reject: ((...args: ReadonlyArray<unknown>) =>
        promiseSettle("reject", args)) as PromisesService["reject"],
      cancel: ((...args: ReadonlyArray<unknown>) =>
        promiseSettle("cancel", args)) as PromisesService["cancel"],
      registerCallback: (...args: Parameters<typeof resonate.promises.registerCallback>) =>
        sdkEffect("promises.registerCallback", true, () =>
          resonate.promises.registerCallback(...args)
        ),
      registerListener: (...args: Parameters<typeof resonate.promises.registerListener>) =>
        sdkEffect("promises.registerListener", true, () =>
          resonate.promises.registerListener(...args)
        )
    ...
    const register = ((
      ...
      registerRaw(
        ...
        funcOrOptions,
        registerOptions
      )) as ResonateClientService["register"]
    const run = ((
      id: string,
    ...
    const service = {
      ...
      schedule: (
        ...
        sdkEffect("schedule", true, () =>
          resonate.schedule(name, cron, func as AnyFunc, ...args)
        ).pipe(Effect.map(scheduleHandle)),
      options: (request?: OptionsInput) =>
        sdkSync("options", false, () => resonate.options(request)),
      promises,
      schedules,
      stop: () => shutdown

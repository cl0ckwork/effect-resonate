...
const packages = packagePaths.map((directory) => ({
  directory,
...
const npmView = (spec) => {
  try {
    return execFileSync("npm", ["view", spec, "version", "--json", "--registry", "https://registry.npmjs.org/"], {
      encoding: "utf8",
...
if (missingPackages.length > 0) {
  console.log(`Skipping automated staging until all packages have been bootstrapped on npm: ${missingPackages.map(({ name }) => name).join(", ")}`)
  process.exit(0)
}
...
for (const { directory, name, version } of packages) {
  if (version === "0.0.0") {
    console.log(`Skipping ${name}@${version}; it has not received its first Changesets version bump.`)
    continue
  }
  ...
  try {
    ...
    const detail = `${error.stdout ?? ""}\n${error.stderr ?? ""}`
    if (!/already (been )?staged|staged version|E409|EPUBLISHCONFLICT|previously published/i.test(detail)) {

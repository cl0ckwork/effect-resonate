...
1. Add `packages/testing/postgres` for the provider-specific runner, harness, and fixtures. Keep Postgres and IntegreSQL client dependencies in the private testing package's development dependencies. Run its Docker script explicitly with `pnpm --filter @effect-resonate/testing test:postgres`, outside regular `pnpm test`.

...
1. Add private `apps/postgres-e2e` workspace package with core, provider, testing, SDK, `pg`, and IntegreSQL client as app-only dependencies. Keep its Docker test script out of the regular root `pnpm test` traversal.
...
5. Run all eight U5 scenarios locally and in the Postgres GitHub Actions job. Verify app typecheck, regular `pnpm check` and `pnpm test`, Docker-backed app test, and Compose teardown. Add concise run instructions and record the SQL provenance.

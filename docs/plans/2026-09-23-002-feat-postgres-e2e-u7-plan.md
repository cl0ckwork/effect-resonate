...
1. Add `packages/testing/src/postgres` for the provider-specific Vitest global setup, harness, and fixtures. Keep Postgres and IntegreSQL client dependencies in the private testing package's development dependencies. Regular `pnpm test` includes Postgres; `SKIP_POSTGRES_TESTS=true` omits it.
...
3. Vendor commit-pinned upstream `resonate.sql` unchanged. Apply it to the IntegreSQL template, then apply the separately documented SDK global-promise compatibility migration. Use IntegreSQL's `hashFiles` for both migrations, upstream metadata, and image configuration. Close the migration connection before template finalization.

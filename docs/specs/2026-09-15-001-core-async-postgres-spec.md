...
- A narrow provider-author seam that accepts an SDK-compatible network factory
  and typed constructor-error mapping that retains the original cause. Workflow, step, context, and client
...
| S7 | Raw persisted payloads, causes/stacks, and schema input are not included in wrapper-owned contract errors or durable values. A thin in-memory `ResonateSdkError` deliberately retains the original SDK `cause`; provider packages pass SDK configuration, methods, errors, and logger output through without sanitization. Applications own diagnostic and logger policy. | W7, W8 | A schema issue echoes a payment token into a durable error; sentinel-based acceptance proves wrapper-owned durable values respect their boundary. |
...
| Configuration → Postgres provider package | Official `PostgresNetworkConfig` | Pass directly to the official SDK constructor. Core never sees provider configuration; the SDK and application own its semantics and diagnostics. |
